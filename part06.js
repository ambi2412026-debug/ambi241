...
window.fbStorage    = storage; /* conservé uniquement comme "truthy" pour les gardes existantes — non utilisé pour l'upload */

/* ══════════════════════════════════════════════════════════════
   CLOUDINARY — remplace Firebase Storage pour tous les nouveaux
   uploads (images + vidéos). Même API que le SDK Storage
   (fbRef / fbUploadBytes / fbUploadBytesResumable / fbGetDownloadURL /
...
function _cldSanitizePublicId(path){
  return String(path || "")
    .replace(/\.[a-zA-Z0-9]+$/, "")        // retire l'extension
    .replace(/[^a-zA-Z0-9/_-]/g, "_");     // caractères sûrs uniquement
}

function _cldUpload(fileOrBlob, path, signal){
  var form = new FormData();
  form.append("file", fileOrBlob);
  form.append("upload_preset", CLOUDINARY_UPLOAD_PRESET);
  if(path) form.append("public_id", _cldSanitizePublicId(path));
  return fetch(CLOUDINARY_UPLOAD_URL, { method: "POST", body: form, signal: signal })
    .then(function(r){
      if(!r.ok){
        return r.json().catch(function(){ return null; }).then(function(j){
          throw new Error((j && j.error && j.error.message) || ("Cloudinary HTTP " + r.status));
        });
      }
      return r.json();
    })
...
/* fbRef(storageIgnored, path) → référence factice */
function _cldRef(_storageIgnored, path){
  return { path: path, _cloudinaryUrl: null };
}

/* fbUploadBytes(ref, file) → Promise<{ref}> (comme uploadBytes) */
function _cldUploadBytes(ref, file){
  return _cldUpload(file, ref && ref.path).then(function(url){
    if(ref) ref._cloudinaryUrl = url;
    return { ref: ref };
  });
}

/* fbGetDownloadURL(ref) → Promise<string> */
function _cldGetDownloadURL(ref){
  if(ref && ref._cloudinaryUrl) return Promise.resolve(ref._cloudinaryUrl);
  return Promise.reject(new Error("Aucune URL Cloudinary disponible pour cette référence"));
}

/* fbUploadBytesResumable(ref, blob) → task-like { on(), cancel(), snapshot } */
function _cldUploadBytesResumable(ref, blob){
  var controller = new AbortController();
  var onError = null, onComplete = null;
  var task = {
    snapshot: { ref: ref, bytesTransferred: 0, totalBytes: (blob && blob.size) || 0 },
    cancel: function(){ try{ controller.abort(); }catch(e){} },
    on: function(_event, _onProgress, errorCb, completeCb){
      onError = errorCb; onComplete = completeCb;
    }
  };
  _cldUpload(blob, ref && ref.path, controller.signal).then(function(url){
    if(ref) ref._cloudinaryUrl = url;
...
/* fbDeleteObject(ref) → suppression désactivée (nécessite une clé secrète
   côté serveur, jamais exposée au navigateur). Ne bloque rien : les
   appelants existants sont déjà en try/catch ou .catch(()=>{}). */
function _cldDeleteObject(ref){
  console.warn("[Cloudinary] Suppression de fichier ignorée (nécessite une signature serveur) :", ref && ref.path);
  return Promise.resolve();
}

window.fbRef                      = _cldRef;
window.fbUploadBytes              = _cldUploadBytes;
window.fbUploadBytesResumable     = _cldUploadBytesResumable;
window.fbGetDownloadURL           = _cldGetDownloadURL;
window.fbDeleteObject             = _cldDeleteObject;
...
// PERF: loadAdminConfig() uniquement si compte admin actif — aucune requête Firestore pour les visiteurs/membres
setTimeout(function(){
  if(isAdmin && typeof loadAdminConfig === "function") loadAdminConfig();
}, 400);
...
var _presUnsubs  = {}; // interval id map pour présences (ex-unsubscribe map onSnapshot)
var _voteUnsubs  = {}; // interval id map pour votes
var _rateUnsubs  = {}; // interval id map pour ratings

/* ══ QUOTA-FIX ═══════════════════════════════════════════════════
   Avant : onSnapshot() sur chaque sous-collection (presences/votes/
   ratings) relisait TOUTE la sous-collection à chaque écriture faite
   par N'IMPORTE QUEL visiteur, pour CHAQUE carte ouverte par CHAQUE
   visiteur → effet boule de neige qui épuise le quota de lectures
   Firestore en quelques heures.
   Après : un simple getDocs() relancé toutes les POLL_MS millisecondes
   par carte. Même traitement, mêmes variables de sortie
   (window._livePresences / _liveVotes / _liveRatings), donc aucune
   fonction d'affichage en aval n'a besoin de changer. Le nombre de
   lectures devient prévisible et borné (cartes affichées × 3 ×
   fréquence), au lieu de dépendre du trafic de tous les visiteurs. */
var _QUOTA_FIX_POLL_MS = 45000; // 45s — à ajuster si besoin (60000 = encore plus économe)

// ── Abonner un établissement à ses flux Firestore (polling) ──
function _subscribeEtab(eid) {
  var eidStr = String(eid);

  // ── Présences (sous-collection) ──────────────────────────────
  if (!_presUnsubs[eidStr]) {
    var fetchPresences = function() {
      var presCol = collection(db, "etablissements", eidStr, "presences");
      getDocs(presCol).then(function(snap) {
        var now = Date.now();
        var TTL = 3 * 3600 * 1000;
        var users = [];
    ...
    fetchPresences(); // premier chargement immédiat (pas d'attente de 45s)
    _presUnsubs[eidStr] = setInterval(fetchPresences, _QUOTA_FIX_POLL_MS);
  }
  ...
  if (!_voteUnsubs[eidStr]) {
    var fetchVotes = function() {
      ...
      getDocs(voteCol).then(function(snap) {
    ...
    fetchVotes();
    _voteUnsubs[eidStr] = setInterval(fetchVotes, _QUOTA_FIX_POLL_MS);
  }
  ...
  if (!_rateUnsubs[eidStr]) {
    var fetchRatings = function() {
      ...
      getDocs(rateCol).then(function(snap) {
        var total = 0, count = 0;
        snap.forEach(function(d) {
          var r = d.data().rating;
    ...
    fetchRatings();
    _rateUnsubs[eidStr] = setInterval(fetchRatings, _QUOTA_FIX_POLL_MS);
  }
}
...
// ── Ne poller que les cartes VISIBLES à l'écran (IntersectionObserver) ────
// PERF PATCH : avant, jusqu'à 15 cartes pollaient en continu (45 req/45s)
// même si l'utilisateur ne les voyait pas en scrollant. Maintenant, seules
// les cartes réellement dans le viewport (+ marge 200px) sont pollées ;
// un délai de grâce de 8s évite de couper/relancer le poll au moindre
// scroll rapide (thrashing).
var _cardPollTimeouts = {};       // eidStr -> timeout de désabonnement différé
var _CARD_UNSUB_GRACE_MS = 8000;  // grâce avant d'arrêter le poll d'une carte sortie de l'écran
var _cardVisObserver = null;

function _unsubscribeEtab(eid) {
  var eidStr = String(eid);
  if (_presUnsubs[eidStr]) { clearInterval(_presUnsubs[eidStr]); delete _presUnsubs[eidStr]; }
  if (_voteUnsubs[eidStr]) { clearInterval(_voteUnsubs[eidStr]); delete _voteUnsubs[eidStr]; }
  if (_rateUnsubs[eidStr]) { clearInterval(_rateUnsubs[eidStr]); delete _rateUnsubs[eidStr]; }
}

function _ensureCardObserver() {
  if (_cardVisObserver) return;
  _cardVisObserver = new IntersectionObserver(function(entries) {
    entries.forEach(function(entry) {
      var eid = entry.target.id.replace('card-etab-', '');
      if (!eid) return;
      if (entry.isIntersecting) {
        if (_cardPollTimeouts[eid]) { clearTimeout(_cardPollTimeouts[eid]); delete _cardPollTimeouts[eid]; }
        _subscribeEtab(eid);
      } else {
        if (_cardPollTimeouts[eid]) clearTimeout(_cardPollTimeouts[eid]);
        _cardPollTimeouts[eid] = setTimeout(function() {
          _unsubscribeEtab(eid);
          delete _cardPollTimeouts[eid];
        }, _CARD_UNSUB_GRACE_MS);
      }
    });
  }, { rootMargin: '200px 0px', threshold: 0.01 });
}

function _observeCard(el) {
  if (!el || el.dataset._pollObserved) return;
  el.dataset._pollObserved = '1';
  _cardVisObserver.observe(el);
}

function _scanAndObserveCards() {
  document.querySelectorAll('.card[id^="card-etab-"]').forEach(_observeCard);
}

// ── Remplace l'ancien abonnement massif : prépare juste l'observer ──
function _subscribeAllEtabs() {
  _ensureCardObserver();
  _scanAndObserveCards();
  // Filet de sécurité : capte aussi les cartes ajoutées après coup
  // (virtualisation au scroll, filtres, changement de vue compacte/complète, etc.)
  if (!window._cardMutationObserver) {
    window._cardMutationObserver = new MutationObserver(function(mutations) {
    ...
    window._cardMutationObserver.observe(document.body, { childList: true, subtree: true });
  }
}

// Appeler dès que Firebase est prêt + après chaque loadData
window._subscribeAllEtabs = _subscribeAllEtabs;
window._unsubscribeEtab = _unsubscribeEtab;
...
// ═══════════════════════════════════════════════════════════════
// Signal que Firebase est prêt (pour les modules qui attendent)
// ═══════════════════════════════════════════════════════════════
